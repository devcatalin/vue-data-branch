...
type VueDataBranchOptions = {
  roots: Root[]
}

const VueDataBranchPlugin: PluginObject<VueDataBranchOptions> = {
  install(Vue, options) {
    if (!options) {
      throw new Error("VueDataBranch: no roots specified.");
    }
    const roots: Root[] = options.roots;
    const rootsByName: Record<string, Root> = {};
      ...
      watch: {
        branch: {
          ...
          handler() {
            ...
            if (root.Root) {
              root.Root(cloneDeep(dataByRootName[root.name]));
            }
...
export default VueDataBranchPlugin;
